...
function LeagueController($scope, $http) {
    ...
    $scope.calculateKDARatio = function (kill, death, assist) {
        var numKill = kill == undefined ? 0 : parseInt(kill);
        var numAssist = assist == undefined ? 0 : parseInt(assist);
    ...
    $scope.AddGameStats = function (gameStats){
        var  temp = {game: gameStats};
        $scope.recentMatches.data.games.push(gameStats);
    }
